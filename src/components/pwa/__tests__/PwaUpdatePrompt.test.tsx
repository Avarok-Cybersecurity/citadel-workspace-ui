...
/**
 * Sonner's removal timer must not outlive the environment.
 *
 * Dismissing a toast does not remove it: Sonner arms a `setTimeout` and removes
 * it when that fires, so it can animate out (`sonner/src/index.tsx:194` ->
 * `removeToast`). Nothing cancels that timer when the Toaster unmounts. When the
 * last test in this file finished with one still pending, vitest tore the jsdom
 * environment down, the timer then fired into nothing, and `setToasts` reached
 * react-dom's `getCurrentEventPriority`, which read `window`.
 *
 * The result was `ReferenceError: window is not defined`, counted as an unhandled
 * error, and the job exited 1 with **3098 tests passed and 0 failed** — a red run
 * naming a file whose every test was green, and pointing at react-dom internals.
 * It reproduced in CI and not locally, because whether the timer beats teardown
 * is a race.
 *
 * Fake timers make the race impossible rather than unlikely: every pending timer
 * is run to completion below while the DOM still exists. `shouldAdvanceTime` keeps
 * the clock moving on its own so `findBy*` and `waitFor` behave normally, and
 * userEvent is given the same clock — without that, `user.click` waits on a timer
 * nothing advances and hangs until the test times out.
 */
function makeUser(): UserEvent {
...
describe('PwaUpdatePrompt', () => {
  ...
  afterEach(() => {
    // Run what is pending, so a toast that was on its way out actually leaves the
    // tree rather than greeting the next test. Measured: most cases end with one
    // timer pending, and about half of those schedule a further one as they run,
    // so this does not reach zero — `useRealTimers` then drops whatever is left.
    //
    // Dropping is the guarantee. A discarded FAKE timer cannot fire at all, which
    // is the entire difference from before: a real one was still queued in the
    // node process when the environment went away.
